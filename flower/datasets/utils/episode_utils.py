...
def process_vision_language(episode: Dict[str, np.ndarray], transforms: Dict, with_vis_lang: bool) -> Dict[str, torch.Tensor]:
    ...
    if with_vis_lang:
        vis_image_static = torch.from_numpy(episode["vis_image_static"]).float().permute(0, 3, 1, 2) # (B, H, W, C) -> (B, C, H, W)
        vis_image_static = transforms["rgb_static"](vis_image_static)
        seq_vis_lang["vis_image_static"] = vis_image_static

        vis_image_gripper = torch.from_numpy(episode["vis_image_gripper"]).float().permute(0, 3, 1, 2) # (B, H, W, C) -> (B, C, H, W)
        vis_image_gripper = transforms["rgb_gripper"](vis_image_gripper)
