...
class DiskDataset(BaseDataset):
    ...
    def _build_file_indices(self, abs_datasets_dir: Path) -> np.ndarray:
        """
        This method builds the mapping from index to file_name used for loading the episodes of the non vision-language
        dataset.
...
class ExtendedDiskDataset(DiskDataset):
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        ...
        if self.with_vis_lang:
            episode["vision_image_static"] = self.vis_image_static[self.vis_lang_lookup[idx]][np.newaxis, :]
            episode["vision_image_gripper"] = self.vis_image_gripper[self.vis_lang_lookup[idx]][np.newaxis, :]
